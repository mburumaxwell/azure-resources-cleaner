...
using AzureCleaner.Purgers;

namespace AzureCleaner;

internal class AzureResourceCleaner(DevOpsPurger devOpsPurger, AzureResourcesPurger azureResourcesPurger, IOptions<AzureResourceCleanerOptions> options)
{
    private readonly AzureResourceCleanerOptions options = options?.Value ?? throw new ArgumentNullException(nameof(options));
...
public class AzureResourceCleanerOptions
{
    ...
    public static implicit operator AzureResourcesPurgeOptions(AzureResourceCleanerOptions options)
    {
